...
import (
	...
	"github.com/vito/git-dagger/querybuilder"
)
...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (any, error) {
	...
	case "GitRepo":
		...
		case "DefaultBranch":
			...
			return (*GitRepo).DefaultBranch(&parent, ctx)
		case "LatestSemverTag":
			...
			if inputArgs["prefix"] != nil {
				err = json.Unmarshal([]byte(inputArgs["prefix"]), &opts.Prefix)
				if err != nil {
					fmt.Println(err.Error())
					os.Exit(2)
				}
		...
		case "Repo":
			...
			return (*GitRepo).Repo(&parent, url), nil
		case "WithBase":
			var err error
			var parent GitRepo
			err = json.Unmarshal(parentJSON, &parent)
			...
			return (*GitRepo).WithBase(&parent, base), nil
		default:
	...
	case "GitUtil":
		...
		case "Base":
			...
			return (*GitUtil).Base(&parent), nil
		case "Repo":
			var err error
			var parent GitUtil
			err = json.Unmarshal(parentJSON, &parent)
	...
	case "":
		return dag.CurrentModule().WithObject(dag.TypeDef().WithObject("GitRepo").WithFunction(dag.NewFunction("Base", dag.TypeDef().WithObject("Container")).WithDescription("Base returns the base image used for git commands.\n")).WithFunction(dag.NewFunction("DefaultBranch", dag.TypeDef().WithKind(Stringkind)).WithDescription("DefaultBranch returns the default branch of a git repository.\n")).WithFunction(dag.NewFunction("LatestSemverTag", dag.TypeDef().WithKind(Stringkind)).WithDescription("LatestSemverTag returns the latest semver tag of a git repository.\n\nIt accepts an optional prefix which can be used to filter tags. For example,\nif you have multiple sub-directories in a monorepo you might have a\nconvention like sub/path/v1.2.3.\n").WithArg("Prefix", dag.TypeDef().WithKind(Stringkind).WithOptional(true), FunctionWithArgOpts{Description: "Prefix to filter tags by."})).WithFunction(dag.NewFunction("Repo", dag.TypeDef().WithObject("GitRepo")).WithDescription("WithRepo sets the repo for future calls to run against.\n").WithArg("url", dag.TypeDef().WithKind(Stringkind))).WithFunction(dag.NewFunction("WithBase", dag.TypeDef().WithObject("GitUtil").WithFunction(dag.NewFunction("Base", dag.TypeDef().WithObject("Container")).WithDescription("Base returns the base image used for git commands.\n")).WithFunction(dag.NewFunction("Repo", dag.TypeDef().WithObject("GitRepo")).WithDescription("WithRepo sets the repo for future calls to run against.\n").WithArg("url", dag.TypeDef().WithKind(Stringkind))).WithFunction(dag.NewFunction("WithBase", dag.TypeDef().WithObject("GitUtil")).WithDescription("WithRepo sets the repo for future calls to run against.\n").WithArg("base", dag.TypeDef().WithObject("Container"))).WithField("CustomBase", dag.TypeDef().WithObject("Container"), TypeDefWithFieldOpts{Description: ""})).WithDescription("WithRepo sets the repo for future calls to run against.\n").WithArg("base", dag.TypeDef().WithObject("Container"))).WithField("GitUtil", dag.TypeDef().WithObject("GitUtil"), TypeDefWithFieldOpts{Description: ""}).WithField("URL", dag.TypeDef().WithKind(Stringkind), TypeDefWithFieldOpts{Description: ""})).WithObject(dag.TypeDef().WithObject("GitUtil").WithFunction(dag.NewFunction("Base", dag.TypeDef().WithObject("Container")).WithDescription("Base returns the base image used for git commands.\n")).WithFunction(dag.NewFunction("Repo", dag.TypeDef().WithObject("GitRepo")).WithDescription("WithRepo sets the repo for future calls to run against.\n").WithArg("url", dag.TypeDef().WithKind(Stringkind))).WithFunction(dag.NewFunction("WithBase", dag.TypeDef().WithObject("GitUtil")).WithDescription("WithRepo sets the repo for future calls to run against.\n").WithArg("base", dag.TypeDef().WithObject("Container"))).WithField("CustomBase", dag.TypeDef().WithObject("Container"), TypeDefWithFieldOpts{Description: ""})), nil
	default:
